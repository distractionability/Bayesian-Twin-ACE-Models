...
parameters {
  ...
  vector[factor_levels_N] share_ac_std;
  vector[factor_levels_N] share_a_of_ac_std;
...
transformed parameters {
  ...
  vector[factor_levels_N] share_ac_re;
  vector[factor_levels_N] share_a_of_ac_re;
  ...
  share_ac_re = share_ac_mu + share_ac_sigma * (share_ac_std - mean(share_ac_std));
  share_a_of_ac_re = share_a_of_ac_mu + share_a_of_ac_sigma * (share_a_of_ac_std - mean(share_a_of_ac_std));
  
  for (cohort_index in 1:cohort_N){
  
    // With an assumption that DZ twins share 0.68 of their genes
    A_share_assortative[cohort_index] = inv_logit(share_a_of_ac_re[factor_levels[cohort_index]]) * inv_logit(share_ac_re[factor_levels[cohort_index]]);
    C_share_assortative[cohort_index] = (1 - inv_logit(share_a_of_ac_re[factor_levels[cohort_index]])) * inv_logit(share_ac_re[factor_levels[cohort_index]]);
    E_share_assortative[cohort_index] = 1 - inv_logit(share_ac_re[factor_levels[cohort_index]]);
